...
const ADMIN_USERS = [
  ...
  { username: 'team',  password: process.env.TEAM_PASSWORD  || 'change_this_too'       },
];
...
app.get('/api/admin/export', requireAuth, async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="whosenearby-waitlist.csv"');
    res.send(csv);
  } catch (err) {
    ...
    if (sendToAll) {
      ...
      for (const user of result.rows) {
        try {
          ...
          results.push({ contact: user.contact, sent: false, error: err.message });
        }
      }
